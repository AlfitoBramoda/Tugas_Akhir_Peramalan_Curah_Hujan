...
\begin{enumerate}[label=\alph*.]
    ...
    \input{tables/bab_4/Nilai-Lambda-Semua-Wilayah.tex}
    Berdasarkan tabel~\ref{table:nilai-lambda-semua-wilayah} nilai $\lambda$ untuk seluruh wilayah berada pada rentang 0.34 - 0.38, yang menunjukkan bahwa data belum stasioner terhadap varians.
    ...
    \input{tables/bab_4/Hasil-Uji-ADF.tex}
    Tabel~\ref{table:hasil-uji-adf} itu menunjukkan bahwa data curah hujan kelima wilayah Surabaya sudah stasioner sepenuhnya.
...
\end{figure}
Berdasarkan Gambar~\ref{fig:hasil-forecasting-utara} model SARIMA $(1,1,0) \times (1,1,0)_{12}$ untuk wilayah Utara menghasilkan pola prediksi yang lebih halus dibandingkan fluktuasi aktual yang sangat ekstrem.
...
Secara keseluruhan, model tidak cukup responsif terhadap perubahan tajam, menghasilkan prediksi yang cenderung terlalu halus, dan memiliki error yang masih tinggi RMSE (3.341) sehingga kurang representatif untuk menggambarkan dinamika curah hujan wilayah Utara.